...
On the answer call, use only the returned rows, groups, metrics, typed value evidence, and text occurrence evidence. Every planned query scope must be represented by at least one selected evidence ID; do not silently omit a scope. Select only typed leaf IDs that contain the value you are asserting:

- A query metric uses exactly `query-<queryId>:metric:<metricId>`.
- A group uses the typed group key ID (`...:key`), specifically `group-<queryId>-<index>:key`, for its key and `group-<queryId>-<index>:metric:<metricId>` for each metric. Selecting a group metric is sufficient; the application adds that same group's trusted key automatically. If you explicitly select a group key and that group has metrics, select a metric from the same group too. A key-only selection is valid only for a metricless group listing.
- A row field uses `row-<queryId>-<rowId>:field:<fieldId>`.
- Text arithmetic uses the supplied paragraph occurrence IDs.

Query, group, and row owner IDs are containers for citations and are never valid value evidence. Never invent `:numeric:<index>` IDs or select an owner ID in a `values` or `calculation` part. Zero is a valid metric or row value: answer it when the query purpose is `count`, and do not turn a zero count or sum into `not_in_source`; that outcome is reserved for a complete `lookup` query with no matches. For grouped answers select the typed metric/value IDs needed to identify and support each asserted group; explicit keys must be paired with a metric from the same group. Global metrics, unrelated row fields, a key without its same-group metric, or a key from one group with a metric from another do not support a grouped metric answer. Select evidence IDs; do not write answer prose, excerpts, labels, operands, or results. The application renders every part and derives deduplicated citations from those IDs. For a calculation, select occurrence or metric IDs in the part's `evidenceIds`; the application resolves values and recomputes the bounded sum, difference, ratio, share, or percentage change. If no returned rows support the requested fact, return `not_in_source`. For a follow-up, use history only to resolve the referent; query results remain the evidence.
