  ...
  implements RunGateRepository<Report>
{
  ...
  async claim(
    ...
    const rows =
      await this.client()`SELECT * FROM claim_analysis_run(${randomUUID()}, ${input.workspaceId}, ${input.key}, ${input.fingerprint}, ${input.ipHash}, ${input.unlocked ? "unlocked" : null}, ${now}, ${config.freeWorkspaceDailyLimit}, ${config.ipDailyLimit}, ${config.unlockedWorkspaceDailyLimit}, ${config.globalDailyLimit}, ${config.receiptTtlMs}, ${config.leaseMs}, ${config.quotaTtlMs})`;
