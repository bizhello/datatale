...
3. Extract at most four standalone numeric facts. Prefer quantities that are meaningful for the user's stated focus, then quantities central to the report.
4. Extract at most three non-numeric observations only when one exact quotation expresses the observation clearly.
5. Use each quotation once. Choose the shortest exact contiguous quotation that still contains all required context.
