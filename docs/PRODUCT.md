...
Input acceptance limits: 2 MiB per file, 2 MiB canonical source JSON, 5,000 data rows, 30 columns and 30,000 text characters. The strictest limit wins; reject instead of silently truncating. XLSX parsing is bounded by 16 MiB actual total archive expansion, 256 entries, 150,030 physical cells, conservative sparse-coordinate ceilings and a 15-second worker deadline. Treat worksheet dimensions as advisory, reject unsafe cell coordinates before dense-array construction, and apply row and column limits to the prepared table after introductory report rows and empty generated tails are removed. The analysis API validates the canonical source and byte limit again even when browser validation passed.
