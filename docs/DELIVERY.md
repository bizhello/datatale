...
**Current state:** DT-INPUT and HeroUI adoption are integrated through `2514d84`. The AI Dashboard candidate now connects CSV/XLSX/text input to bounded AI analysis, checked facts, a responsive narrative dashboard, and atomic guest/quota/idempotency controls. It also includes one anonymous daily trial, invite-code access with an atomic ten-call daily code budget, an explicitly approximate stepped HeroUI progress indicator, a circular reduced-motion-safe theme transition, and Cyrillic font pairing. Independent review rounds rejected lifecycle, grounding, calculation, SQL migration, progress-semantics, UI, and component-ownership issues; corrections include adversarial regressions and real PostgreSQL probes. Sol approved the progress candidate `6b724ae` and the prior integrated candidate `27f42e8`; local static, unit, build, browser, visual, and live Spiro table/text probes pass. Isolated Neon deployment verification, Vercel preview, and production credential/runtime verification remain required before its PR. Chat, durable history, onboarding, and long-lived source/report persistence are not implemented.
...
| DT-05 durable report storage | Luna executor; conductor integrates | active | base `0985a70`; `feat/dt-05-report-storage`; `../datatale-worktrees/report-storage` | Database schema/migrations and new persisted-analysis entity/repository paths; no chat UI, root config or canonical docs | Deliver owner-scoped immutable source/report persistence and message-ready storage contracts with PostgreSQL/in-memory tests |
| DT-08 grounded chat core | Luna executor; conductor integrates | active | base `0985a70`; `feat/dt-08-chat-core`; `../datatale-worktrees/chat-core` | New chat entity contracts and `features/ask-data/server`; no DB schema, route, UI, root config or canonical docs | Deliver injected, source-grounded answer service with exact insufficient-data result and adversarial tests |
| DT-08 grounded chat UI | Luna executor; conductor integrates | active | base `0985a70`; `feat/dt-08-chat-ui`; `../datatale-worktrees/chat-ui` | `features/ask-data/model` and `features/ask-data/ui`; no server, DB, app route, global CSS, dependency files or canonical docs | Deliver responsive HeroUI conversation/composer states behind an injected send boundary, with component tests |
