...
All Must Have features, the optional functional tour, owner-scoped history/reopen, perceptible four-stage analysis transition, daily workspace access tiers, saved-report recovery, Russian analysis copy, production-shell cleanup, strengthened AI prompt contracts, required chart recovery, grounded report summaries, large-value chart axes, bounded arbitrary-question chat retrieval, inflected categorical-value overviews, responsive input/footer corrections, first-run showcase access, bounded 5,000-row XLSX import, and validation-guided long-text repair are delivered on the current candidate. Migrations `0001`–`0006` remain current. The guarded production build runs the migration ledger before Next.js. Branch and pull-request Vercel deployments remain disabled.

The current candidate gate passes Biome, Steiger, strict TypeScript, the Turbopack production build, 421 Vitest tests, and 81 Playwright scenarios across desktop Chromium, mobile Chromium, and mobile WebKit. Production returns HTTPS 200 and renders the input cards and empty-workspace explanation. A history read without a sealed guest cookie returns private, uncached `401 {"code":"expired"}` without creating a cookie. Production and live-provider probes verified suitable-table charts, grounded report-summary selection, the exact insufficient-data refusal, and the accepted long-text fixture through its full source. The production access smoke created an isolated guest workspace and accepted today's locally derived code without exposing the code or seed.
