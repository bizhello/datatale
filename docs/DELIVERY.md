...
All Must Have features, the optional functional tour, owner-scoped history/reopen, perceptible four-stage analysis transition, daily workspace access tiers, saved-report recovery, Russian analysis copy, production-shell cleanup, strengthened AI prompt contracts, required chart recovery, grounded report summaries, large-value chart axes, full-source model-planned chat queries, responsive input/footer corrections, first-run showcase access, bounded 5,000-row XLSX import, one-call quotation-backed text reports with charts, and localized-date chat evidence are delivered in production. Migrations `0001`–`0006` remain current. The guarded production build runs the migration ledger before Next.js. Branch and pull-request Vercel deployments remain disabled.

The release gate passes Biome, Steiger, strict TypeScript, the Turbopack production build, 413 Vitest tests, and 81 Playwright scenarios across desktop Chromium, mobile Chromium, and mobile WebKit. Production acceptance on `datatale.bizhov.ru` used the versioned fixtures in `tests/manual`: two CSV sources, both worksheets of a multi-sheet XLSX, pasted TXT, malformed CSV, and unsupported TSV. Every accepted source rendered two or three grounded charts. Twenty chat questions verified totals, category and city filters, maxima, comparisons, Russian localized dates, and the exact absence response. The TXT flow returned 21 animals, 7 examined cats, and the 16 September maximum of 8,900 rubles from the complete source. The malformed CSV and TSV produced readable input errors without starting analysis.
