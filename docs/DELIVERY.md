...
All Must Have features, the optional functional tour, owner-scoped history/reopen, perceptible four-stage analysis transition, daily workspace access tiers, saved-report recovery, Russian analysis copy, production-shell cleanup, strengthened AI prompt contracts, required chart recovery, grounded report summaries, large-value chart axes, full-source model-planned chat queries, responsive input/footer corrections, first-run showcase access, bounded 5,000-row XLSX import, validation-guided long-text repair, and quotation-backed text charts are delivered on the current candidate. Migrations `0001`–`0006` remain current. The guarded production build runs the migration ledger before Next.js. Branch and pull-request Vercel deployments remain disabled.

The current candidate gate passes Biome, Steiger, strict TypeScript, the Turbopack production build, 395 Vitest tests, and 81 Playwright scenarios across desktop Chromium, mobile Chromium, and mobile WebKit. Live Spiro probes on this candidate verified checked `3 + 2 = 5` text chat, canonical absence, a quotation-backed text chart, a three-chart report over the real 4,500-row workbook, and a 560-row Краснодар summary from that workbook. The production site still represents the last merged release until this candidate is merged and deployed; post-deploy browser smoke remains part of release completion.
