...
| Large-source chat | Arbitrary row lookup stays below the context byte budget; inflected categorical values resolve without short/common-token false matches; unmatched questions still reach grounded refusal; truncated retrieval cannot be treated as exhaustive |
