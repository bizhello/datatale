...
- Production Neon `datatale-db` is connected in `iad1`; migrations `0001`–`0004` are applied. The next main deployment will apply `0005_strict_report_hero.sql`, which deletes reports outside the current 2–3-item hero contract before adding database constraints that reject future drift.
