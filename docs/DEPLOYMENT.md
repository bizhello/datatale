...
- Neon `datatale-db` uses Free, region `iad1`, with Auth disabled. Connected only to Vercel Production. The candidate requires all four reviewed migrations, but they have not been applied or tested against an isolated preview branch. Production migration remains blocked on exact-candidate review and preview verification.
