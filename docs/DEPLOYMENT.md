...
- Production Neon `datatale-db` is connected in `iad1`; ordered migrations are applied automatically before each production build. Migration `0005_strict_report_hero.sql` deletes reports outside the current 2–3-item hero contract before adding database constraints that reject future drift.
