...
For tables, the model first receives a bounded profile with field types, common categorical values, and question-matched candidates collected across all accepted rows. It returns one flat query wire envelope. Code decodes that envelope into the stricter entity-owned `DatasetQuery`, validates fields, types, filters, grouping, ordering, metrics, and limits, and executes it over the complete dataset. Semantically invalid wire values remain available to one query-only repair call instead of failing before the repair boundary. Existence questions cannot use an unfiltered count as absence evidence; the repaired plan must filter or group the requested entity. A verified zero-row result returns the canonical refusal without another model call. The answer call otherwise receives only bounded rows/groups, exact deterministic metrics, coverage, and trusted query/row references. Ungrouped aggregate answers cite the synthetic query-result reference; grouped answers may cite the exact group whose typed metric and date evidence supports the claim. Russian month-and-year wording is checked against cited ISO dates. The model cannot return `not_in_source` before an exhaustive query: a premature refusal enters one repair call whose schema permits only a replacement query. Provider prose and references on a final missing-source outcome are discarded in favor of the canonical refusal.
