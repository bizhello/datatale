...
Text analysis uses one low-reasoning structured proposal call. The response contains at most eight exact-quotation observations, up to three chart groups, and a 2–3 item narrative plan. Narrative items select bounded templates and observation IDs rather than returning free prose. Application code validates the exact quotes and numeric values, safely normalizes only optional metadata, renders Russian sentences from checked observations, and calculates every chart point. Chart titles and rationales are also deterministic. A schema-valid or source-invalid proposal receives at most one complete validation-guided replacement; partial reports are never published.

Numeric observations use explicit `snapshot`, `change`, or `target` roles. Chart validation enforces unique references, compatible units and period contexts, role/derivation/operation matrices, and signed increase/decrease direction. Qualitative observations remain quotation evidence and do not create artificial numeric charts. The complete original text remains available to chat even though the dashboard proposal is bounded to eight observations.
...
- Compute table facts and all chart points deterministically. Text metrics may reproduce only exact numeric values from validated quotations.
...
For text sources, the model receives the complete accepted text as stable evidence chunks of at most 1,000 characters within the 30,000-character source bound. For an answer it selects typed `quote`, `values`, or `calculation` evidence IDs; it may also ask one focused clarification question, report genuine absence, or identify an unsupported operation. The server renders the Russian answer and canonical citations from those IDs. Provider-authored answer prose, excerpts, labels, operands, and results are not accepted.

For tables, the model first receives a bounded profile with field types, common categorical values, and question-matched candidates collected across all accepted rows. It returns one flat strict wire envelope containing a bounded batch of one to four query objects. Code decodes each object into the stricter entity-owned `DatasetQuery`, validates fields, types, filters, grouping, ordering, metrics, and limits, and executes every query over the complete dataset. One query is preferred when global metrics and groups cover the question; separate scopes or groupings use the bounded batch. Semantically invalid wire values remain available to one batch repair call instead of failing before the repair boundary. Existence questions cannot use an unfiltered count as absence evidence; the repaired plan must filter or group the requested entity. Partial batch results remain available to the answer stage. A verified zero-row result returns the canonical refusal only when every lookup query is empty. The answer call receives only bounded rows/groups, exact deterministic metrics, coverage, and trusted query/row references. Its answer is one to eight typed parts (`quote`, `values`, or `calculation`) containing IDs only; application code resolves labels, operands, expressions, values, and deduplicated references. Provider prose, labels, operands, results and references are discarded. One bounded answer repair is allowed after the batch. Russian month-and-year wording is checked against cited ISO dates.

The query executor supports equality, exclusion, membership, ranges and string containment; selection; one grouping dimension; ordering; count, sum, average, minimum, maximum and distinct count. It scans before limiting and records matched, scanned, returned and truncated coverage. One chat request may contain a bounded batch of up to four independent dataset queries; each scope must be represented by selected evidence in the final answer. A separate bounded arithmetic contract recomputes sums with 2–8 operands and requires exactly two operands for differences, ratios, shares, and percentage changes. It rejects incompatible units and zero denominators and permits only the checked result in the answer. It never executes SQL, JavaScript, Python or model-generated code.
...
Use the configured Spiro OpenAI-compatible gateway and server-side model IDs. The server-only provider adapter in `shared/lib/ai` passes the base URL and key explicitly. Table analysis, narrative, and chat use `AI_MODEL`; the combined text-report proposal uses optional `AI_TEXT_MODEL`, defaulting to `gpt-5.6-luna`, with low reasoning effort. Paid analysis fails closed unless provider, Neon, session, rate-salt and quota settings all exist; deletion and cleanup do not depend on provider availability. AI SDK transport retries are disabled. A table uses at most plan + one repair + narrative (three calls). A valid text report uses one provider call; one complete validation-guided replacement is allowed before failing closed. Text chat uses one provider call. Table chat uses one query-plan call, one query-answer call, and may add one bounded repair at either boundary. Inactive provider fields are canonicalized to their empty sentinels before semantic processing. If a final answer still selects unknown evidence after the bounded repair, the server may construct a bounded deterministic values answer from the trusted query results; it never falls back to provider prose or untrusted IDs. Table planning, repair, and narrative calls have a 45-second stage deadline; the complete two-call chat workflow has a 60-second deadline. The combined text-report call receives 75 seconds, the complete analysis is bounded by 165 seconds, and the serverless route allows 180 seconds for a possible replacement. Every stage retains its own output-token cap, and model strings and serialized reports/results are bounded. Neon atomically enforces five free or twenty unlocked workspace analyses per UTC day plus hashed-IP and global safeguards. Saved-analysis chat separately enforces five free or twenty unlocked user messages across all reports in the workspace per UTC day. Usage before unlock remains charged; assistant replies and idempotent replays are free. Receipts expire after 15 minutes, analysis-run leases and chat inference leases after 90 seconds, saved analyses/messages after seven days from creation, workspaces after 30 days of inactivity, and quota buckets after 48 hours. The primary provider boundary uses Spiro with `gpt-5.6-terra`; the combined text proposal defaults to `gpt-5.6-luna`. Every changed structured-output contract or model route requires a fresh live-model and production smoke before release; mocked tests do not prove model behavior.
