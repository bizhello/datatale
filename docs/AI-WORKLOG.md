...
**Local verification:** `bun run check` passed Biome over 111 files, Steiger, strict TypeScript, 24 Vitest files with 103 tests, and the Next production build. `bun run test:e2e` passed 30 cases across desktop Chromium, mobile Chromium, and mobile WebKit. These checks use controlled provider/repository boundaries; isolated Neon, Vercel preview, production gateway credentials, scheduled cleanup, and versioned live-model quality evaluation remain mandatory before release.
