...
## Inflected categorical chat lookup · 2026-09-20

The real 4,500-row XLSX contained 560 rows whose `Регион` value was `Краснодар`, yet the broad question `дай информацию по краснодару` returned the missing-information refusal. Row retrieval compared normalized tokens too literally, so the dative form `Краснодару` did not retrieve the canonical source value. Even with retrieval repaired, a broad request did not specify which of several numeric columns should be aggregated.

The correction reuses the conservative Russian noun matcher in bounded row retrieval and adds a deterministic overview only when one informative categorical field/value matches unambiguously. It reports the complete checked row count and includes matching values only from already validated charts; it never guesses an aggregation. Independent review found that explicit disqualifier lists still missed year, negation, named-month, row-list and qualitative comparison clauses. The final shortcut allows only a small overview vocabulary plus words that match the selected category; every other term uses the existing grounded provider path. Short/common values are ignored, ambiguous matches stay on that provider path, and truly absent values still produce the exact refusal. The exact workbook passed through the application parser as 4,500 rows and 13 columns, and the corrected path returned the checked 560-row result for Краснодар.
