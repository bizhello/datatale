...
Independent Astra review rejected earlier green candidates for substring numeric validation (`3` matching `30`), sign loss, missing grouped aggregate evidence, truncated long paragraphs, hidden rare categories, duplicate chart observations, double-negated decreases, query type failures outside repair, timeout misclassification, and a chat lease equal to the provider deadline. The corrections introduced typed numeric evidence, complete text chunking, rare Russian-inflection candidates, entity-owned preflight validation, distinct failure outcomes, and a 90-second inference lease. The oversized chat service was then split by responsibility into provider contract, source context, answer validation, arithmetic, and orchestration modules.
...
## 2026-09-21 — production fixture acceptance and localized-date chat correction

The reproducible acceptance pack adds two CSV sources, a two-sheet XLSX workbook, a pasted-text report, malformed CSV, and unsupported TSV. Independent control totals and twenty source questions exercise charts, aggregations, filters, maxima, comparisons, and exact absence responses through the visible production UI.

The first full production run exposed a validator defect: the text model correctly answered that the shelter held 21 animals by 17 September and cited the exact paragraph, but the application returned HTTP 502 because localized answer dates were checked only against ISO evidence. The correction compares explicit Russian and ISO dates in both directions, validates both ends of Russian date ranges, and lists supported Russian month forms instead of accepting arbitrary shared stems such as `мартышка` or `августейший`. Independent review found and drove the bidirectional, history-hydration, word-form, and range cases before approving the exact release SHA.

The same walkthrough showed that technical reference excerpts such as internal column IDs and metric JSON were rendered as evidence chips. Fresh and restored messages now share one neutral evidence-label mapper; server-side excerpts remain available for grounding and validation but are no longer presented as product copy. The deployed acceptance run then completed all supported fixtures with two or three charts each, correct grounded answers, exact absence responses, and readable rejection states for malformed CSV and unsupported TSV.

## 2026-09-21 — final saved-report submission check

The final visible production walkthrough reopened the saved `operations-multisheet.xlsx` support worksheet and a saved pasted-text shelter report. Both restored the source summary, Russian narrative, metrics, rendered charts, evidence, and prior grounded chat transcript without another analysis request. The restored XLSX report showed 500 requests, 453 resolved requests, average CSAT 4.47, three team comparison charts, and the persisted Alpha answer of 250. The restored text report showed quotation-backed animal metrics, two charts, the checked total of 21, and the canonical refusal for rabbits.

An additional Beta question was submitted from the reopened XLSX report. The workspace had already consumed its 20-message daily allowance, and the UI displayed the expected quota exhaustion state. No fresh provider answer after reopen is claimed from this final pass. Browser coverage remains the evidence for post-reopen request wiring, while earlier production checks established live grounded follow-up behavior before quota exhaustion.
