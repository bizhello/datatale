...
**Correction verification:** the updated text case passed in desktop Chromium, mobile Chromium, and mobile WebKit. In a disposable checkout at `e13220c`, the same mobile-WebKit test failed because `.page-shell` lacked `data-hydrated="true"`, establishing that the new regression assertion distinguishes the base. `bun run check` passed Biome, Steiger, strict types, 49 Vitest tests, and the production build; `bun run test:e2e` passed all 27 browser cases. Hosted CI and exact-candidate review remain pending.
